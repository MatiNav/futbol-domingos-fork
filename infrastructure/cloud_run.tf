...
resource "google_cloud_run_service" "futbol-domingos-nextjs-cloud-run-service" {
  ...
  template {
    spec {
      containers {
        image = "us-central1-docker.pkg.dev/${var.project_id}/cloud-run-source-deploy/${var.cloud_run_service_name}:latest"
...
resource "google_cloud_run_service_iam_member" "public_access" {
  service  = google_cloud_run_service.futbol-domingos-nextjs-cloud-run-service.name
  location = google_cloud_run_service.futbol-domingos-nextjs-cloud-run-service.location
